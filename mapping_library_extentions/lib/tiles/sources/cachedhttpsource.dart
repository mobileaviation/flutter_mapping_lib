...
class CachedHttpTileSource extends HttpTileSource
{
  ...
  get DatabaseOpen { return _databaseOpen; }
  ...
  _onCreate(Database db, int version) async {
    await db.execute("CREATE TABLE tiles (TileX INTEGER, TileY INTEGER, TileZ INTEGER, Timestamp REAL, TileBytes BLOB)");
  }
  ...
  Future<void> _insertImageInCache(Tile tile, TileImage tileImage) async {
    ...
    String sql = "INSERT INTO tiles (TileX, TileY, TileZ, TileBytes, Timestamp) ";
    sql = sql + "VALUES(?, ?, ?, ?, julianday('now'))";
    return await _db.execute(sql, [tile.tileX, tile.tileY, tile.zoomLevel, tileImage.data]);
  }
  ...
  Future<TileImage> GetTileImage(Tile tile) async
  {
    if (_databaseOpen) {
      ...
      if (dbimage != null) {
        return await TileImage(dbimage.image, dbimage.data);
      } else {
