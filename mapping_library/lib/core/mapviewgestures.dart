...
class MapViewGestures extends MapViewStateBase {
  ...
  _mapScaleStart(ScaleStartDetails scaleStartDetails) {
    ...
    _touchedOffset = Offset(scaleStartDetails.localFocalPoint.dx, scaleStartDetails.localFocalPoint.dy);
  ...
  _mapScaleUpdate(ScaleUpdateDetails scaleUpdateDetails) {
    GeoPoint newCenterPoint = _dragViewport.GetNewCenterGeopointForDragPosition(_touchedOffset, scaleUpdateDetails.localFocalPoint);
    MapPosition newMapPosition = MapPosition.fromGeopointScale(newCenterPoint, _scale);

    double s = MercatorProjection.zoomLevelToScaleD(newMapPosition.getZoom() + (scaleUpdateDetails.scale - 1));
  ...
  _mapTap(TapUpDetails tapUpDetails) {
    ...
    GeoPoint tp = widget.viewport.GetGeopointForScreenPosition(new math.Point(tapUpDetails.localPosition.dx,
