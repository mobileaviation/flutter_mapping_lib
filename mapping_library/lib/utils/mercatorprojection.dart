...
int getMapSizeWithScale(double scale) {
  return (Tile.SIZE.toDouble() * (math.pow(2, scaleToZoomLevel(scale)))).floor();
}
 ...
 */
math.Point getPixelRelative(GeoPoint geoPoint, int mapSize, double x, double y) {
  double pixelX = longitudeToPixelXMapSize(geoPoint.getLongitude(), mapSize) - x;
 ...
 * @return the relative pixel position to the origin values (e.g. for a tile)
 */
math.Point getPixelRelativeByPoint(GeoPoint geoPoint, int mapSize, math.Point origin) {
  return getPixelRelative(geoPoint, mapSize, origin.x, origin.y);
}

//Point getPixelRelativeToTile(GeoPoint geoPoint, Tile tile) {
...
//double groundResolution(MapPosition pos) {
...
int latitudeToTileY(double latitude, int zoomLevel) {
  return pixelYToTileY(latitudeToPixelYZoomLevel(latitude, zoomLevel), zoomLevel);
}
...
int longitudeToTileX(double longitude, int zoomLevel) {
  return pixelXToTileX(longitudeToPixelXZoomLevel(longitude, zoomLevel), zoomLevel);
}
...
int pixelXToTileX(double pixelX, int zoomLevel) {
  return math.min(math.max(pixelX / Tile.SIZE, 0).floor(), math.pow(2, zoomLevel) - 1);
}
...
int pixelYToTileY(double pixelY, int zoomLevel) {
  return math.min(math.max(pixelY / Tile.SIZE, 0).floor(), math.pow(2, zoomLevel) - 1);
}
...
//math.Point project(GeoPoint p, math.Point reuse) {
//  double x = ((p.longitudeE6 / 1E6) + 180.0) / 360.0;
...
double tileXToLongitude(int tileX, int zoomLevel) {
  return pixelXToLongitude((tileX * Tile.SIZE).toDouble(), getMapSize(zoomLevel));
}
...
double tileYToLatitude(int tileY, int zoomLevel) {
  return pixelYToLatitude((tileY * Tile.SIZE).toDouble(), getMapSize(zoomLevel));
}
