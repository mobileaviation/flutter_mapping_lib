...
/// Convert locations (degrees/doubles) to E6 values
int degreeToE6(double value) {
  return (value * CONVERSION_FACTOR).round();
}

double E6ToDegree(int value) {
...
List<math.Point> interceptOnCircle(
    math.Point p1, math.Point p2, math.Point c, double r) {
  ...
  var underRadical = math.pow(r, 2) * math.pow(m, 2) +
      math.pow(r, 2) -
      math.pow(b, 2); //the value under the square root sign

  if (underRadical < 0) {
    //line completely missed
    return null;
  } else {
    var t1 = (-m * b + math.sqrt(underRadical)) /
        (math.pow(m, 2) + 1); //one of the intercept x's
    var t2 = (-m * b - math.sqrt(underRadical)) /
        (math.pow(m, 2) + 1); //other intercept's x
