...
class GeoPoints extends ListBase<p.GeoPoint> {
  ...
  _setupMinMax(p.GeoPoint point) {
    ...
    _maxLat = (_maxLat == 1000) ? lat : math.max(_maxLat, lat);
  ...
  String getGeoPointsStr() {
    String list = "";
    for (p.GeoPoint point in _innerList) {
      String latlon = point.getLongitude().toString() + "," + point.getLatitude().toString();
      list = list + latlon + " ";
    }
    return list.trimRight();
  }
}
